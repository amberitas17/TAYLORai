...
class EAPExhibitDataset(Dataset):
    """Dataset for EAP exhibit classification"""

    def __init__(self, data_dir, transform=None):
        self.data_dir = data_dir
        self.transform = transform

        # Get EAP exhibit classes
        eap_dir = os.path.join(data_dir, "EAP")
        self.classes = sorted([d for d in os.listdir(eap_dir)
                             if os.path.isdir(os.path.join(eap_dir, d))])
        ...
        for class_name in self.classes:
            class_dir = os.path.join(eap_dir, class_name)
        ...
        print(f"EAP Dataset loaded: {len(self.image_paths)} images")
        print(f"Number of EAP exhibit types: {self.num_classes}")
...
def train_eap_classifier(data_dir='hierarchical_dataset', num_epochs=60, batch_size=16,
                        learning_rate=0.001, img_size=224, use_pretrained=True):
    """Train EAP-specific exhibit classifier"""

    print("=== EAP Exhibit Classifier Training ===")
    ...
    # Load dataset
    train_dataset = EAPExhibitDataset(data_dir, transform=train_transform)
    val_dataset = EAPExhibitDataset(data_dir, transform=val_transform)
    ...
    if use_pretrained:
        try:
            ...
            if os.path.exists(main_model_path):
                ...
                # Create temporary model with 3 classes to load weights
    ...
    print("\nStarting EAP classifier training...")
    print("=" * 80)
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'best_eap_classifier.pth')
    ...
    # Save final model
    torch.save(model.state_dict(), 'final_eap_classifier.pth')
    ...
    model_info = {
        ...
        'model_type': 'EAP_classifier'
    }
    torch.save(model_info, 'eap_classifier_info.pth')

    # Save as JSON for easy reading
    with open('eap_classifier_info.json', 'w') as f:
    ...
    plt.title('EAP Classifier Training and Validation Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    ...
    plt.title('EAP Classifier Training and Validation Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig('eap_classifier_training_history.png', dpi=150, bbox_inches='tight')
    plt.show()

    print(f"\nEAP Classifier training completed!")
    print(f"Best validation accuracy: {best_val_acc:.2f}%")
    print(f"Models saved: best_eap_classifier.pth, final_eap_classifier.pth")
    print(f"Model info saved: eap_classifier_info.pth, eap_classifier_info.json")

    return model, model_info

if __name__ == "__main__":
    train_eap_classifier(
