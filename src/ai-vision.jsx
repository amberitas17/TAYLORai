...
import { faceAnalysisService, FaceAnalysisResult } from './services/faceAnalysisService.js';
import { useFaceVerification } from './contexts/FaceVerificationContext.jsx';
import { personDetectionService } from './services/personDetectionService.js';
...
export default function FaceVerification() {
  ...
  useEffect(() => {
    ...
    // Start eye animation for AI personality
    ...
    // Skip person detection and go directly to face analysis
  ...
  const startRapidPersonDetection = async () => {
    ...
    console.log('🚀 Starting rapid person detection...');
    
    detectionInterval.current = setInterval(async () => {
      if (detectionActive && cameraRef.current && entertainmentPhase === 'detecting') {
        try {
          ...
          if (photo.base64) {
            const result = await personDetectionService.detectPersonFromBase64(photo.base64);
            
            if (result.person_detected && result.confidence > 0.3) {
              console.log('👋 Person detected! Starting entertainment sequence...');
    ...
    }, 500); // Check every 500ms for rapid response
  };

  const startEntertainmentSequence = async () => {
    // Skip greeting phase and go directly to analyzing to prevent camera on/off
    ...
    try {
      if (cameraRef.current) {
        ...
        if (photo.base64) {
          ...
          // Handle mixed success/failure - continue if we have any emotion data
          if (!analysisResult.success) {
            // Check for partial success (emotion detection worked even if age prediction failed)
          ...
          // Auto-proceed to AI chat for personalized experience
          setTimeout(() => {
  ...
  // Greeting Phase - AI detected someone
  ...
  // Removed hidden camera to prevent conflicts
